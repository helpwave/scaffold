...
function ScaffoldNodeInner({ id, data, selected }: NodeProps<Node<ScaffoldNodeData>>) {
    ...
    return (
        <div
            className={`rounded-lg border overflow-hidden shadow min-w-[160px] bg-[#ffffff] dark:bg-[#0a0a0a] px-3 py-2 [&_.react-flow__handle]:!bg-[#ffffff] [&_.react-flow__handle]:dark:!bg-[#0a0a0a] ${selected
                ? 'border-indigo-500 dark:border-indigo-400 ring-2 ring-indigo-500/20 dark:ring-indigo-400/20'
                : 'border-gray-300 dark:border-gray-600'
                }`}
        >
            {showTargetHandle && (
                <Handle type="target" position={Position.Top} className="!min-w-[14px] !min-h-[14px] !w-4 !h-4 !border-1 !border-gray-300 dark:!border-gray-600 !bg-[#ffffff] dark:!bg-[#0a0a0a]" />
            )}
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="nodrag flex items-center shrink-0 gap-0.5">
                        {hasChildren && (
                            <IconButton
                                onClick={(e) => {
                                    e.stopPropagation()
            ...
            {showSourceHandle && (
                <Handle type="source" position={Position.Bottom} className="!min-w-[14px] !min-h-[14px] !w-4 !h-4 !border-1 !border-gray-300 dark:!border-gray-600 !bg-[#ffffff] dark:!bg-[#0a0a0a]" />
            )}
        </div>
    )
}
